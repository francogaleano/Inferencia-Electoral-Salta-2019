### Inferencia Ecologica para las elecciones a Gobernador 2019 en Salta
...
## Cargo Base de datos
# Procurar que no tenga NA's (esto ya lo revisé)
data1<-read.csv("data/base_inferencia_ecologica_saltaGob2019.csv", sep = ";")


dsPpre <- data1[,7:14] #De que col a que col van los rtados de las paso
dsPpre <- dsPpre/data1[,6] #n de col de electores
dsPpre <- cbind(dsPpre,1-rowSums(dsPpre))
dsPpre <- as.matrix(dsPpre)

dsGpre <- data1[,15:19] #de que col a que col van los rtados de las grales
dsGpre <- dsGpre/data1[,6] #n de col de electores
dsGpre <- cbind(dsGpre,1-rowSums(dsGpre))
dsGpre<- as.matrix(dsGpre)

#Paso a General
newdata<-as.matrix(cbind(dsPpre,dsGpre))
estsPG<-params.estim(newdata,nR=9,nC=6) #9 filas (1 para blancos/ausentes), 6 cols (1 para blancos/ausentes)
fracsPG<-calc.fractions(estsPG,nR=9,nC=6)
colnames(fracsPG)<-colnames(dsGpre)
rownames(fracsPG)<-colnames(dsPpre)

##Datos Summary
VotosPaso <- data1[,7:14]
VotosPaso <- cbind(VotosPaso,data1[,6]-rowSums(VotosPaso))
VotosGeneral <- data1[,15:19]
VotosGeneral <- cbind(VotosGeneral ,data1[,6]-rowSums(VotosGeneral))
...
indice <- tibble(indice = c("Paso Saenz", "Paso Leavy (FDT)",
                            "Paso Isa (FDT)","Paso Olmedo",
                            "Paso Villegas (FIT)", "Paso Gil (FIT)","Paso Lopez (FIT)",
                            "Paso Fernandez", "Paso No votó/Blanco"))

tabla %>%
  as_tibble() %>% 
  bind_cols(indice) %>%
  select(indice, everything()) %>%
  rename(grales_novoto_blanco = `1 - rowSums(dsGpre)`) %>% 
